...
bool AVL::InsertName(AVL_Node *pNew){
	...
	while(pWalker){ //while pWalker is not NULL
		...
		if(pWalker->compareName(pNew)>0){ //if new data is smaller than data of pWalker
			pWalParrent=pWalker;
			pWalker=pWalker->getChild(NAME_LEFT); //go to LEFT child
		}
		else if(pWalker->compareName(pNew)<0){ //if new data is bigger than data of pWalker
			pWalParrent=pWalker;
	...
	if(pWalParrent->compareName(pNew)>0){ //set LEFT child
	...
	if(pCurrent->compareName(pNew)>0){
	...
	while(pWalker!=pNew){
		if(pWalker->compareName(pNew)>0){ //height of LEFT added
...
bool AVL::Delete(char* firstName, char* lastName, char* subKey){
	...
	while(pWalker){ //while pWalker is not NULL
		...
		if(pWalker->compareName(firstName, lastName, subKey)>0){ //if find data is smaller than data of pWalker
			pWalParrent=pWalker;
			pWalker=pWalker->getChild(NAME_LEFT); //go to LEFT child
		}
		else if(pWalker->compareName(firstName, lastName, subKey)<0){ //if find data is bigger than data of pWalker
			pWalParrent=pWalker;
			pWalker=pWalker->getChild(NAME_RIGHT); //go to RIGHT child
		}
		else{ //find node
	...
	if(!(pDelete->getChild(NAME_LEFT))&&!(pDelete->getChild(NAME_RIGHT))){ //if pDelete has no child
		...
		if(pWalParrent->compareName(pDelete)>0){
		...
		while(pCurrent!=pWalParrent){
			if(pCurrent->compareName(pDelete)>0){
		...
		if(pWalParrent==pDelete){ //if pWalParrent is pDelete
			if(pDelete->compareName(pWalker)>0){
		...
		else if(pWalParrent->compareName(pWalker)>0){
		...
		while(pCurrent!=pWalParrent){
			if(pCurrent->compareName(pWalParrent)>0){
				pCurrent->setNameFactor(pCurrent->getNameFactor()-1);
				pCurrent=pCurrent->getChild(NAME_LEFT);
			}
			else{
...
bool AVL::PrintAll(FILE_IO& file){
	if(!root_name){
		return false;
	}

	PrintName(NULL, NULL, NULL, file);
	PrintPhone(NULL, file);

	return true;
}

bool AVL::PrintName(char* lastName, FILE_IO& file){
	if(!PrintName(NULL, lastName, NULL, file)){
		return false;
	}
	return true;
}

bool AVL::PrintName(char* firstName, char* lastName, FILE_IO& file){
	if(!SearchOverlap(firstName, lastName)){
		return false;
	}
	if(!PrintName(firstName, lastName, NULL, file)){
		return false;
	}
	return true;
}

bool AVL::PrintName(char* firstName, char* lastName, char* subKey, FILE_IO& file){
	stack<AVL_Node*> avlStack;
	AVL_Node *pWalker=root_name;

	if(!root_name){ //no data
		return false;
	}
	if(firstName&&!subKey&&!SearchOverlap(firstName, lastName)){ //overlap data without subkey
		return false;
	}
	...
	while(true){
		...
		else if(lastName){ //print last name
...
bool AVL::PrintPhone(char* phoneNum, FILE_IO& file){
	stack<AVL_Node*> avlStack;
	...
	while(true){
		...
		else if(!strcmp(pWalker->getPhoneNum(),phoneNum)){ //print node
				file.WriteData(0, pWalker->getFirstName(), pWalker->getLastName(), pWalker->getPhoneNum(), pWalker->getAddress());
			}
