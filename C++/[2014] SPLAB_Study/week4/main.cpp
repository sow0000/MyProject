...
int main(){
	...
	for(int i=0;i<100000;i++){
		while(1){
			x=(rand()%20000)*0.0001-1;
			y=(rand()%20000)*0.0001-1;
			r=range(x,y,0,0);
			if((y<=rt/2)&&(y>=-rt/2)&&(y<=(1+x)*rt)&&(y<=(1-x)*rt)&&(y>=(-1+x)*rt)&&(y>=(-1-x)*rt)&&(r!=0)) break;
		}
		p=power(r);
		in=0;
...
double power(double r){
	double m=(double)(-1)*log((rand()%9999)*0.0001+0.0001);
	double p=((double)20000/1024)*pow((double)10,1.5)*pow((double)10,-11.1)*pow(r,-3.7)*m;
	return p;
}
